...
export const adminLogin = asyncHandler(async (req, res) => {
  ...
  if (validatingEmail.password == password && validatingEmail.isAdmin == true) {
...
export const updateUser = asyncHandler(async (req, res) => {
  ...
  if(!validatingAdminId) return res.status(403).json({error:"forbidden not access to update"})
  ...
  if (!updateUser) return res.status(404).json({ error: "user Not found",user:rest });
...
export const createNewUser=asyncHandler(async(req,res)=>{
  ...
  if(!name&&!password&&!email) return res.status(400).json({error:"invalid required Data"})
  ...
  console.log(hashingPassword,"is the password")
    ...
    res.status(201).json({message:"new user Created successFully",user:rest})
})
